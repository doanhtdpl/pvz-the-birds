...
namespace PlantsVsZombies.Plants_Bullets.Grows
{
    public abstract class GrowButton : Button
    {
        ...
        public bool CanBuy
        {
            get
            {
                return (!this.isCoolDowning && (this.Manager.SunManager.NumberOfSuns >= this.Price));
            }
        }
        #endregion

        #region Constructors
        public GrowButton(GrowManager manager, Sprite background, Sprite foreground, Vector2 position)
            : base(manager.Game, background, foreground, position)
        {
        ...
        public override void Draw(GameTime gameTime)
        {
            if (isCoolDowning)
            {
                this.coolDownImage.Position = new Vector2(this.controlBackground.PositionX, this.controlBackground.PositionY + this.controlBackground.SizeX - this.coolDownImage.SizeY);
        ...
        private void OnCoolDownTick(Counter.ICounter timer)
        {
            ...
            this.coolDownImage.SizeY = this.coolDownImage.Texture.Width;
        }

        private void OnVisualCoolDownTick(Counter.ICounter timer)
        {
            this.coolDownImage.SizeY -= this.coolDownImage.Texture.Width / 20f;
        }
        ...
        public Plant.Plant Buy()
        {
            ...
            this.Manager.SunManager.NumberOfSuns = this.Manager.SunManager.NumberOfSuns - this.Price;
